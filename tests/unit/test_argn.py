...
class TestMakePermutationMask:
    ...
    def test_sequential_any_order(self):
        ...
        # append tgt:/ column
        sidx_ridx_col_dim = 2
        col_embedding_dims += [sidx_ridx_col_dim]
        columns += ["tgt:/"]
        ...
        assert mask[0, :sidx_ridx_col_dim].equal(torch.Tensor([False, False]))
        assert mask[1, :sidx_ridx_col_dim].equal(torch.Tensor([True, True]))
        assert mask[2, :sidx_ridx_col_dim].equal(torch.Tensor([True, True]))
        assert mask[3, :sidx_ridx_col_dim].equal(torch.Tensor([True, True]))
