...
def analyze_language_datetime(values: pd.Series, root_keys: pd.Series, _: pd.Series | None = None) -> dict:
    ...
    min_n = min_dates.sort_values(ascending=True).head(ANALYZE_N_MIN_MAX).astype(str).tolist()
    max_dates = df.groupby(root_keys.name)[values.name].max().dropna()
    max_n = max_dates.sort_values(ascending=False).head(ANALYZE_N_MIN_MAX).astype(str).tolist()
