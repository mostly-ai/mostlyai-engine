...
def analyze_numeric(
    ...
    min_n = min_values.sort_values(ascending=True).head(ANALYZE_N_MIN_MAX).astype("float").tolist()
    max_values = df.groupby(root_keys.name)[values.name].max().dropna()
    max_n = max_values.sort_values(ascending=False).head(ANALYZE_N_MIN_MAX).astype("float").tolist()
