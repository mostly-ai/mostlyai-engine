...
def load_base_model_and_config(
    ...
    if is_training and device.type == "cuda" and importlib.util.find_spec("bitsandbytes") is None:
        _LOG.warning(
            "CUDA device was found but bitsandbytes is not available. Please use `pip install -U 'mostlyai-engine[gpu]'` to install bitsandbytes for quantization."
        )
