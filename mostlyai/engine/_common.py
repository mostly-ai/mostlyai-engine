...
def load_probability_data(workspace_dir: str | Path) -> pd.DataFrame:
    """
    Load probability data from workspace directory.

    Probability data is generated by `predict_proba()` and contains model probability
    distributions for target variables in exploded format (one column per category/bin/value).

    Args:
        workspace_dir: Path to the workspace directory.

    Returns:
        DataFrame containing probability distributions with exploded columns.
    ...
    Raises:
        FileNotFoundError: If no probability files are found in the workspace.
    """
    workspace_dir = Path(workspace_dir)
    probability_data_path = workspace_dir / "ProbabilityData"

    # Read all parquet files from ProbabilityData directory
    parquet_files = sorted(probability_data_path.glob("*.parquet"))
    if not parquet_files:
        raise FileNotFoundError(f"No parquet files found in {probability_data_path}")
