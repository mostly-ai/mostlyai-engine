...
SIDX_SLEN_COLUMN = f"{TGT}{PREFIX_TABLE}{PREFIX_COLUMN}"
SIDX_SLEN_DIGIT_ENCODING_THRESHOLD = 100
SIDX_SUB_COLUMN_PREFIX = f"{SIDX_SLEN_COLUMN}{PREFIX_SUB_COLUMN}sidx_"  # sequence index
SLEN_SUB_COLUMN_PREFIX = f"{SIDX_SLEN_COLUMN}{PREFIX_SUB_COLUMN}slen_"  # sequence length
...
def encode_slen_sidx(vals: pd.Series, max_seq_len: int, prefix: str = "") -> pd.DataFrame:
    assert is_integer_dtype(vals)
    if max_seq_len < SIDX_SLEN_DIGIT_ENCODING_THRESHOLD:
...
def decode_slen_sidx(df_encoded: pd.DataFrame, max_seq_len: int, prefix: str = "") -> pd.Series:
    if max_seq_len < SIDX_SLEN_DIGIT_ENCODING_THRESHOLD:
...
def get_slen_sidx_cardinalities(max_seq_len) -> dict[str, int]:
    ...
    # order is important: sidx first, slen second, as the former has highest priority
...
def trim_sequences(syn: pd.DataFrame, tgt_context_key: str, seq_len_min: int, seq_len_max: int):
    ...
    syn.drop(
        [c for c in syn.columns if c.startswith(SIDX_SLEN_COLUMN)],
        axis=1,
        inplace=True,
    )
