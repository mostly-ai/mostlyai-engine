...
def _enrich_slen_sidx(df: pd.DataFrame, context_key: str, max_seq_len: int) -> pd.DataFrame:
    ...
    slen = encode_slen_sidx(slen, max_seq_len=max_seq_len, prefix=SLEN_SUB_COLUMN_PREFIX)
    sidx = encode_slen_sidx(sidx, max_seq_len=max_seq_len, prefix=SIDX_SUB_COLUMN_PREFIX)
    df = pd.concat([slen, sidx, df], axis=1)
    return df
