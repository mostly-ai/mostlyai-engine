...
parser.add_argument("--branch", type=str, default="fix-pos-correl")
parser.add_argument("--random_state", type=int, default=42)
parser.add_argument("--dataset", type=str, default="toy-long")
...
seed_data = seed_data_arg = args.seed_data
experiment_name = (
...
match dataset:
    ...
    case "usb":
        ctx = pd.read_parquet("usb_data/ctx.parquet")[:1_200]
        tgt = pd.read_parquet("usb_data/tgt.parquet")
        tgt_context_key = "id"
        ctx_primary_key = "id"
        tgt = tgt[tgt[tgt_context_key].isin(ctx[ctx_primary_key])]
    case "cdnow":
    ...
    case "toy-long":
        tgt = pd.read_parquet("toy_long.parquet")
        ctx_primary_key = tgt_context_key = "id"
        ctx = tgt[[tgt_context_key]].drop_duplicates()
        ctx = pd.concat([ctx, pd.DataFrame({ctx_primary_key: [100000]})], ignore_index=True)
    case _:
...
# Add max_epochs, dataset, and seed to the plot title
plot_title = f"Sequence Length Distribution (%)\nmax_epochs={max_epochs}, dataset={dataset}, seed_data={seed_data_arg}"
